...
if [ "$suffix" = '-full' ];then
    [ "$repo_name" == 'lede' ] && rootfs_size=$( awk -F= '/^CONFIG_TARGET_ROOTFS_PARTSIZE/{print $2+32}' .config )
    [[ "$repo_name" =~ 'DHDAXCW' ]] && rootfs_size=$( awk -F= '/^CONFIG_TARGET_ROOTFS_PARTSIZE/{print $2+24}' .config )
fi
